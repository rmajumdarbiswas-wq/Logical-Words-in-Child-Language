...
all_Spanish_tokens$gloss <- all_Spanish_tokens$gloss %>% tolower()

# count the tokens before exclusions
n_tokens_before_exclusions <- nrow(all_Spanish_tokens)

# number of children before exclusions
n_chi_before_exclusions <-
  all_Spanish_tokens$target_child_id %>% unique() %>% length()
...
#n_unintelligibles_removed <- nrow(Spanish_tokens)

# number of children after excluding unintelligible tokens
#n_chi_unintelligibles_removed <-
#  Spanish_tokens$target_child_id %>% unique() %>% length()
...
#n_tokens_babble_removed <- nrow(Spanish_tokens)
...
n_tokens_unreasonable_removed <- nrow(Spanish_tokens)
...
Spanish_tokens <- 
  Spanish_tokens %>% drop_na(target_child_age)

# count the tokens after removing NA tokens
n_tokens_ageNAs_removed <- nrow(Spanish_tokens)
...
n_tokens_ageOver72mo_removed <- nrow(Spanish_tokens)
...
write_csv(Spanish_tokens, "../raw_data/Spanish_tokens_excluded.csv")
```
...
Spanish_tokens$speaker <- "parent"
Spanish_tokens$speaker[Spanish_tokens$speaker_role=="Target_Child"] <- "child"
```
...
```{r age}
Spanish_tokens$age <- Spanish_tokens$target_child_age %>% floor()
```
...
Spanish_tokens$utterance_type <-
  recode(Spanish_tokens$utterance_type, 
...
Spanish_tokens_contract <-
  rbind(Spanish_tokens_contract, contracted_contractions)
...
# Spanish_tokens_contract is what you want to run the functions on to analyze contractions seperate from their stems
```
...
# Defining Function Words in Spanish
...
```{r FunctionWords}
Spanish_tokens_contract$word <- "nonfunction"

function_words_contract <- c("  "no", "sí", "y", "o", "si", "ni", "por_tanto", "ninguno", "alguno", "cada", "todo", "mayoría", "poco", "mucho", "varios", "ambos","alguien", "nadie", "algo", "todo", "ningún_lugar", "algún_lugar", "todas_partes",  "más", "menos", "mucho", "más", "menos", "que",  "uno", "dos", "tres", "cuatro", "cinco", "seis", "siete", "ocho", "nueve", "diez",  "primero", "segundo", "tercero", "cuarto", "quinto",  "sexto", "séptimo", "octavo", "noveno", "décimo", "último",  "poder", "necesitar", "deber", "quizá", "tal_vez", "ir", "querer",  "ningún", "nadie", "nada", "nunca", "todavía",  "el", "la", "los", "las", "un", "una",  "este", "ese", "estos", "esos",  "siempre", "nunca", "a_veces", "ahora", "a_menudo", "una_vez", "dos_veces",  "cuando", "mientras", "después", "antes", "entonces", "hasta", "desde", "durante",  "quién", "cuándo", "qué", "de_quién", "dónde", "cómo", "por_qué",  "en", "sobre", "fuera", "arriba", "abajo", "debajo", "encima",  "detrás", "a_través", "al_lado", "entre", "más_allá", "dentro","cerca", "hacia", "aquí", "allí", "porque", "pero", "aunque", "ser", "estar", "haber", "yo", "tú", "nosotros", "él", "ella", "ellos", "me", "nos", "te", "lo", "la", "los", "las", "mi", "tu", "su", "nuestro",  "otra_vez", "también", "además", "otro", "otros", "todavía",  "solo", "justo", "incluso", "de_hecho","tampoco", "ni", "si", "casi", "ya", "excepto", "para", "de", "en_lugar_de", "con", "sin", "sobre", "por", "muy", "a", "de", "en", "contra"")

for (x in function_words_contract){
  Spanish_tokens_contract$word[Spanish_tokens_contract$gloss==x] <- x
}

# create separate function words that do include the function words as their own words
Spanish_tokens$word <- "nonfunction"
...
Spanish_tokens$word_category <- "nonfunction"

logical_connectives <- c("no, no, y, o, si, ni, por lo tanto, por tanto") 

for (x in logical_connectives){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Logicals"
}


Spanish_tokens_contract$word_category <- "nonfunction"

logical_connectives <- c("no, no, y,o, si, ni, por lo tanto, por tanto")

for (x in logical_connectives){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Logicals"
}
```
...
for (x in quantifiers){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Quantifiers"
}

for (x in quantifiers){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Quantifiers"
}
...
for (x in quantity){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Quantity"
}

for (x in quantity){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Quantity"
}
```
...
for (x in cardinals){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Cardinals"
}

for (x in cardinals){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Cardinals"
}
...
for (x in ordinals){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Ordinals"
}

for (x in ordinals){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Ordinals"
}
```
...
for (x in modals){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Modals"
}

for (x in modals){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Modals"
}
...
for (x in NPI){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "NPI"
}

for (x in NPI){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "NPI"
}
```
...
for (x in definites){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Definites"
}

for (x in definites){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Definites"
}
```
...
for (x in demonstratives){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Demonstratives"
}

for (x in demonstratives){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Demonstratives"
}
```
...
for (x in temporal_quantifiers){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Temporal Quantifiers"
}

for (x in temporal_quantifiers){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Temporal Quantifiers"
}
```
...
for (x in temporal_connectives){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Temporal Connectives"
}

for (x in temporal_connectives){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Temporal Connectives"
}
```
...
for (x in Wh){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Wh"
}

for (x in Wh){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Wh"
}
```
...
for (x in locatives){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Locatives"
}

for (x in locatives){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Locatives"
}
```
...
for (x in causatives){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Causal Connectives"
}

for (x in causatives){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Causal Connectives"
}
```
...
for (x in contrasts){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Contrast Connectives"
}

for (x in contrasts){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Contrast Connectives"
}
...
for (x in contrasts){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "auxiliaries"
}

for (x in contrasts){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "auxiliaries"
}
```
...
for (x in pronouns){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Pronouns"
}

for (x in pronouns){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Pronouns"
}
```
...
for (x in temporal_connectives){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Additives"
}

for (x in temporal_connectives){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Additives"
}
```
...
for (x in exclusives){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Exclusives"
}

for (x in exclusives){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Exclusives"
}
```
...
for (x in focus){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Focus Particles"
}

for (x in focus){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Focus Particles"
}
```
...
for (x in other){
  Spanish_tokens$word_category[Spanish_tokens$gloss==x] <- "Other"
}

for (x in other){
  Spanish_tokens_contract$word_category[Spanish_tokens_contract$gloss==x] <- "Other"
}
```

```{r}
Spanish_tokens$syncategory <- "Function"
Spanish_tokens$syncategory[Spanish_tokens$word_category=="nonfunction"] <- "nonfunction"

Spanish_tokens_contract$syncategory <- "Function"
Spanish_tokens_contract$syncategory[Spanish_tokens_contract$word_category=="nonfunction"] <- "nonfunction"
```

## Saving Summary Tables

```{r save_dataframe}
write_feather(Spanish_tokens, "../processed_data/Spanish_tokens_processed.feather")
write_feather(Spanish_tokens_contract, "../processed_data/Spanish_tokens_contract_processed.feather")
```
